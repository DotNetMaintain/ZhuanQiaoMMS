...
namespace RM.Busines.DAL
{
    public class RM_UserInfo_Dal : RM_UserInfo_IDAO
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("Select User_ID,User_Account,User_Pwd,User_Name,DeleteMark,User_Remark from Base_UserInfo where ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT *  FROM Base_UserInfo WHERE User_ID = @User_ID");
            SqlParam[] para = new SqlParam[]
			{
				new SqlParam("@User_ID", User_ID)
			};
