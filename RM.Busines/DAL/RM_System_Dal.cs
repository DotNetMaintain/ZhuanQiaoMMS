...
namespace RM.Busines.DAL
{
    public class RM_System_Dal : RM_System_IDAO
    {
        ...
        public DataTable InitMaterialType()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT valuecode,valuename from Base_DictionaryInfo WHERE TypeName = '物资类型' ORDER BY ValueCode ASC");
            return DataFactory.SqlDataBase().GetDataTableBySQL(strSql);
        }


       public DataTable InitMaterialStatus(string DicType)
       {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT valuecode,valuename from Base_DictionaryInfo WHERE TypeName = '"+DicType+"'   ORDER BY ValueCode ASC");
