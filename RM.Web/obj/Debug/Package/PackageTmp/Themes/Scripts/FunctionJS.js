...
function deliveryConfig(url, parm) {
    showConfirmMsg('注：您确认要发放这些物资吗？', function (r) {
        if (r) {
            getAjaxJason(url, parm, function (rs) {
                if (!rs) {
                    showTipsMsg("发放成功！", 2000, 4);
                    windowload();
                } else if (length(rs) == 0) {
                    showTipsMsg("发放失败，0 行受影响！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>发放失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
...
function restoreConfig(url, parm) {
    showConfirmMsg('注：您确认要退回这些物资吗？', function (r) {
        if (r) {
            getAjaxJason(url, parm, function (rs) {
                if (!rs) {
                    showTipsMsg("退回成功！", 2000, 4);
                    windowload();
                } else if (length(rs) == 0) {
                    showTipsMsg("退回失败，0 行受影响！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>退回失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
